/**
 * Given a string S that consists of only alphanumeric characters and dashes. The string is separated into N + 1
 * groups by N dashes. Also given an integer K.
...
fun format(s: String, k: Int): String {
    var output = ""
    val cleanedString = s.replace("-", "").uppercase()

    var counter = k
    for (i in cleanedString.length - 1 downTo 0) {
        if (counter == 0) {
            counter = k
            output += "-"
        }
        output += cleanedString[i]
        counter--
    }
...
fun main() {
    ...
    val s1 = "2-5g-3-J2-5ss-as-212-44-cqsacs-ca"
    val k1 = 5

    println(format(s1, k1))
    println("---------------")

    val s2 = "2-5g-3-J2-5ss-as-212-44-cqsacs-ca"
    val k2 = 7

    println(format(s2, k2))
}